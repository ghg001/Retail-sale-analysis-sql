...
WHERE 
    ...
    quantity >= 4
```
3. ** Numbers of  sales for each category.**:
```sql
SELECT 
    category,
...
```
5. ** Total_sale is greater than 1000.**:
```sql
SELECT * FROM retail_sales
WHERE total_sale > 1000
```

6. ** The total number of transactions made by each gender in each category.**:
```sql
SELECT 
    category,
    gender,
...
LIMIT 5
```
9. ** Numbers of unique customers who purchased items from each category.**:
```sql
SELECT 
    category,    
